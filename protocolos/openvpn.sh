...
# Protocolo           : TCP
# Red VPN             : 10.8.0.0/24
# Configuración       : /etc/yaku-multi-script/config.conf
# ==============================================================
...
title() {
    ...
    echo -e "${CYAN}╔══════════════════════════════════════════════════════════════╗${RESET}"
    echo -e "${CYAN}║${RESET}              ${MAGENTA}${BOLD}🛡️ YAKU OPENVPN${RESET}                  ${CYAN}║${RESET}"
...
check_ubuntu() {
    ...
    if [[ "$ID" != "ubuntu" ]]; then
        error_msg "Yaku-Multi-Script OpenVPN requiere Ubuntu."
        return 1
    fi
...
[Unit]
Description=Yaku-Multi-Script OpenVPN Firewall

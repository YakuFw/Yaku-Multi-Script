...
header(){
    clear_screen
    echo -e "${CYAN}╔══════════════════════════════════════════════════════════════╗${RESET}"
    echo -e "${CYAN}║${RESET} ${PINK}${BOLD}              🛡️ YAKU CHECKUSER${RESET}                ${CYAN}║${RESET}"
...
create_checkgestor(){
    ...
    chmod 755 /bin/checkgestor
    mkdir -p /etc/yaku-multi-script/limits
...
[Unit]
Description=Yaku-Multi-Script CheckUser API
...
[Unit]
Description=Yaku-Multi-Script Online App
...
create_license_info(){
    mkdir -p "$LICENSE_DIR"
    printf '%s\nYaku-Multi-Script CheckUser Manager\n' "$LICENSE_TEXT" > "$LICENSE_FILE"
    chmod 644 "$LICENSE_FILE"
}
...
show_original_install_info(){
    ...
    echo -e "\E[44;1;37mVERSIÓN 1.6       Integrado Yaku-Multi-Script \E[0m"
    echo
...
show_result(){
    ...
    echo
    ok "Yaku-Multi-Script CheckUser quedó instalado."
    pause
}
...
while true; do
    ...
    echo
    echo -e "${GRAY}Yaku-Multi-Script • CheckUser Manager v5.1${RESET}"
    echo

...
def env():
 global API,OWNER
 d={}
 if not ENV.exists():raise SystemExit('Falta /etc/yaku-multi-script/telegram/.env')
...
def api_multipart(method,fields,files,timeout=60):
 boundary='----YakuMultiScriptBoundary'+str(int(time.time()*1000));body=bytearray()
...
I18N.update({
 ...
 'info':{
  'es':'ℹ️ <b>CENTRO DE INFORMACIÓN</b>\n\nYaku-Multi-Script reúne en un solo bot la gestión de cuentas, conexiones, renovaciones, referidos, configuración y herramientas del servidor. Las opciones visibles dependen de tus permisos.',
...
VERSION_URL='https://raw.githubusercontent.com/YakuFw/Yaku-Multi-Script/main/version.txt'
def current_version():
 try:
...
def new_version():
 try:
  req=urllib.request.Request(VERSION_URL,headers={'User-Agent':'Yaku-Multi-Script-Updater'})
...
def run_update(c=None,auto=False,key=None):
 ...
 if not updater.exists():
  ...
  return
 cmd=f'bash {q(updater)} --telegram' if c else f'bash {q(updater)}'
 if c:
...
def process_text(c,t,chat_type=None):
 ...
 st0=STATE.get(c)
 if st0 and st0.get('f') in ('monetag','domain','admin_add','admin_remove','admin_rename','ban_add','ban_remove','message_users','quota_public','quota_admin'):
...
def cb(c,m,u,i,x,chat_type=None):
 ...
 if x.startswith('tool:'):
  ...
  if k=='files':return edit(c,m,'📁 <b>ARCHIVOS</b>\n\n<pre>'+e(subprocess.getoutput("find /etc/yaku-multi-script -maxdepth 2 -type f | sort | head -120"))+'</pre>',TOOLS)
 ...
 while True:
  try:
   ...
   if mode!='once' and next_at<=time.time() or mode=='once' and next_at and next_at<=time.time():
    fn=backup_now();
    try:send_document(OWNER,fn,'💾 <b>Respaldo automático de Yaku-Multi-Script</b>')
